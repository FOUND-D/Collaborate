export const SESSION_LIST_REQUEST = 'SESSION_LIST_REQUEST';
export const SESSION_LIST_SUCCESS = 'SESSION_LIST_SUCCESS';
export const SESSION_LIST_FAIL = 'SESSION_LIST_FAIL';

export const SESSION_DETAILS_REQUEST = 'SESSION_DETAILS_REQUEST';
export const SESSION_DETAILS_SUCCESS = 'SESSION_DETAILS_SUCCESS';
export const SESSION_DETAILS_FAIL = 'SESSION_DETAILS_FAIL';

export const SESSION_CREATE_REQUEST = 'SESSION_CREATE_REQUEST';
export const SESSION_CREATE_SUCCESS = 'SESSION_CREATE_SUCCESS';
export const SESSION_CREATE_FAIL = 'SESSION_CREATE_FAIL';
export const SESSION_CREATE_RESET = 'SESSION_CREATE_RESET';

export const SESSION_CONFIRM_REQUEST = 'SESSION_CONFIRM_REQUEST';
export const SESSION_CONFIRM_SUCCESS = 'SESSION_CONFIRM_SUCCESS';
export const SESSION_CONFIRM_FAIL = 'SESSION_CONFIRM_FAIL';

export const SESSION_CANCEL_REQUEST = 'SESSION_CANCEL_REQUEST';
export const SESSION_CANCEL_SUCCESS = 'SESSION_CANCEL_SUCCESS';
export const SESSION_CANCEL_FAIL = 'SESSION_CANCEL_FAIL';

export const SESSION_COMPLETE_REQUEST = 'SESSION_COMPLETE_REQUEST';
export const SESSION_COMPLETE_SUCCESS = 'SESSION_COMPLETE_SUCCESS';
export const SESSION_COMPLETE_FAIL = 'SESSION_COMPLETE_FAIL';

export const SESSION_STATUS_RESET = 'SESSION_STATUS_RESET';
