...
export const register = (name, email, password, image, department, role, yearOfStudy, studentId) => async (dispatch) => {
  try {
    dispatch({
    ...
    const { data } = await api.post(
      '/api/users/register',
      { name, email, password, profileImage: image, department, role, yearOfStudy, studentId },
      config
    );
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await api.get(
      id === 'profile' ? '/api/users/profile' : `/api/users/${id}`,
      config
    );
